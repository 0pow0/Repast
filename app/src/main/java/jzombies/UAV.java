...
public class UAV {
	...
	// socket
	// private static ActionInfoBuilder aiBuilder = new ActionInfoBuilder();
	private static ROCBuilder rocb = new ROCBuilder();
	private static Socket socket;
	static {
		try {
			Parameters params = RunEnvironment.getInstance().getParameters();
			int port = params.getInteger("socket_port");
			UAV.socket = new Socket("127.0.0.1", port);
		} catch (Exception e) {
	...
	static {
		try {
			UAV.os = new DataOutputStream(UAV.socket.getOutputStream());
		} catch (Exception e) {
			System.out.println(e);
		}
	}
	...
	static {
		try {
			UAV.is = new DataInputStream(UAV.socket.getInputStream());
		} catch (Exception e) {
			System.out.println(e);
		}
	}
	...
	public UAV(Geography<Object> geography, double height, double speed,
			boolean if_straight_path, boolean if_routing_path, int start_time, double tx_power, double rx_threshold,
			boolean if_nearest_target, String mode, Map<String, ArrayList<Double>> airport_list, Operator my_operator,
	...
	// Random Constructor
	public UAV(Geography<Object> geography, double height, double speed, boolean if_straight_path,
			boolean if_routing_path, int start_time, double tx_power, double rx_threshold, boolean if_nearest_target,
			String mode, Map<String, ArrayList<Double>> airport_list, int current_batch, int level, boolean unmanned,
			int id, long num_ch) {
		this.geography = geography;
		this.height = getMeters(height);
		this.speed = speed;
	...
	private static ByteBuffer readSocket(InputStream is) {
		try {
			byte[] len = new byte[4];
			byte[] bytes;
			ByteBuffer buf;
	...
	private static SINRResp readSINRFromSocket(InputStream is) {
		try {
			ROCInfo roc = ROCInfo.getRootAsROCInfo(readSocket(is));
	...
	private static ThroughputResp readThroughputFromSocket(InputStream is) {
		try {
			ROCInfo roc = ROCInfo.getRootAsROCInfo(readSocket(is));
			int rocType = roc.infoType();
			if (rocType == ROCType.ThroughputResp) {
	...
	private static EnbResp readEnbFromSocket(InputStream is) {
		try {
			ROCInfo roc = ROCInfo.getRootAsROCInfo(readSocket(is));
			int rocType = roc.infoType();
			if (rocType == ROCType.EnbResp) {
				EnbResp resp = (EnbResp) roc.info(new EnbResp());
				System.out.println("eNB ID = " + resp.enbId());
				System.out.println("Used RB = " + resp.usedRb());
				return resp;
			} else
				return null;
		} catch (Exception e) {
	...
	public void CreatSocket(Double lat_1, Double lng_1) {
		if (this.id > -1 && this.id != 999) {
			String uav_id = String.valueOf(this.id);
			// Geometry pt = geography.getGeometry(this);
			String lat = String.valueOf(lat_1);
			String lng = String.valueOf(lng_1);
			String delay = String.valueOf(this.internal_time_step) + "s";
			// System.out.println("ID:"+this.id);
			byte[] buf = UAV.rocb.buildCreationInfo(delay, uav_id, lat, lng, 5);
			try {
				System.out.println("[ CreationInfo ] Before write to socket ID = " + this.id);
				UAV.os.write(buf, 0, buf.length);
				UAV.os.flush();
				System.out.println("[ CreationInfo ] After write to socket");
			} catch (Exception e) {
				System.out.println(e);
			}
		}
	...
	private static void requireForThroughputReq(OutputStream os, ROCBuilder rocb, String uav_id, String delay) {
		byte[] buf = rocb.buildThroughputReq(delay, uav_id);
		try {
			System.out.println("[ ThroughputReq ] Before write to socket ID = " + uav_id);
			os.write(buf, 0, buf.length);
			os.flush();
			System.out.println("[ ThroughputReq ] After write to socket");
		} catch (Exception e) {
			System.out.println(e);
		}
	}

	private static void requireForSINRRequest(OutputStream os, ROCBuilder rocb, String uav_id, String delay) {
		byte[] buf = rocb.buildSINRReq(delay, uav_id);
		try {
			System.out.println("[ SINRReq ] Before write to socket ID = " + uav_id);
			os.write(buf, 0, buf.length);
			os.flush();
			System.out.println("[ SINRReq ] After write to socket");
		} catch (Exception e) {
			System.out.println(e);
		}
	}

	private static void requireForRBRequest(OutputStream os, ROCBuilder rocb, String enb_id, String delay) {
		byte[] buf = rocb.buildEnbReq(delay, enb_id);
		try {
			System.out.println("[ RB Usage ] Before write to socket ID = " + enb_id);
			os.write(buf, 0, buf.length);
			os.flush();
			System.out.println("[ RB Usage ] After write to socket");
		} catch (Exception e) {
			System.out.println(e);
		}
	}
				...
				if (this.movestatus == true) {
					if (distance_x < curr_uav.speed && distance_y < curr_uav.speed) {
						...
						if (this.id > -1 && this.id != 999) {
							...
							String delay = String.valueOf(this.internal_time_step) + "s";
							System.out.println("ID:" + this.id);
							byte[] buf = UAV.rocb.buildDeletionInfo(delay, uav_id);
							try {
								System.out.println("[ DeletionInfo ] Before write to socket");
								UAV.os.write(buf, 0, buf.length);
								UAV.os.flush();
								System.out.println("[ DeletionInfo ] After write to socket");
							} catch (Exception e) {
								System.out.println(e);
							}
						}
						...
						if (this.id == 999 && this.internal_time_step % 5 == 0) {
							...
							for (Object obj : base_stations) {
								...
								requireForRBRequest(UAV.os, UAV.rocb, String.valueOf(temp.return_bs_id()), delay);
								EnbResp resp_Enb = readEnbFromSocket(UAV.is);
								save_Enb(resp_Enb);
							}

						}

						if (this.id > -1 && this.internal_time_step % 5 == 0 && this.id != 999) {
							String uav_id = String.valueOf(this.id);
							String lat = String.valueOf(pt.getCoordinate().y);
							String lng = String.valueOf(pt.getCoordinate().x);
							String delay = String.valueOf(this.internal_time_step) + "s";
							byte[] buf = UAV.rocb.buildActionInfo(delay, uav_id, lat, lng);
							try {
								System.out.println("[ DeletionInfo ] Before write to socket ID = " + this.id);
								UAV.os.write(buf, 0, buf.length);
								UAV.os.flush();
								System.out.println("[ DeletionInfo ] After write to socket");
							} catch (Exception e) {
							...
							requireForSINRRequest(UAV.os, UAV.rocb, uav_id, delay);
							SINRResp resp_SINR = readSINRFromSocket(UAV.is);
							save_SINR(resp_SINR);
							requireForThroughputReq(UAV.os, UAV.rocb, uav_id, delay);
							ThroughputResp resp_Throughtput = readThroughputFromSocket(UAV.is);
							save_Throughput(resp_Throughtput);
						}
						if (this.internal_time_step % 1 == 0) {
							if (curr_uav.unmanned) {
								save_location("1");
				...
				if (this.movestatus == false) {
					...
					if (distance_x < curr_uav.speed && distance_y < curr_uav.speed) {
						...
						if (this.id > -1 && this.internal_time_step % 5 == 0 && this.id != 999) {
							...
							String delay = String.valueOf(this.internal_time_step) + "s";
							// System.out.println("ID:"+this.id);
							// System.out.println("Lat:"+pt.getCoordinate().y);
							// System.out.println("Lng:"+pt.getCoordinate().x);
							byte[] buf = UAV.rocb.buildActionInfo(delay, uav_id, lat, lng);
							try {
								System.out.println("[ ActionInfo ] Before write to socket ID = " + this.id);
								UAV.os.write(buf, 0, buf.length);
								UAV.os.flush();
								System.out.println("[ ActionInfo ] After write to socket");
							} catch (Exception e) {
								System.out.println(e);
							}
	...
	// save UAV SINR in each time tick
	private void save_Enb(EnbResp resp) {
		if (resp == null)
			return;
		...
		String fileName = temp_pre_fix + File.separator + "report" + File.separator + "batch_"
				+ Integer.toString(current_batch) + File.separator + "bs_Enb" + ".csv";

		try {
			PrintWriter writer_uav_SINR = new PrintWriter(new FileOutputStream(new File(fileName), true));
			StringBuilder uav_SINR_string = new StringBuilder();
			...
			uav_SINR_string.append(internal_time_step);
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.usedRb());
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.enbId());
			uav_SINR_string.append('\n');

			writer_uav_SINR.write(uav_SINR_string.toString());
			writer_uav_SINR.flush();
			writer_uav_SINR.close();
	...
	private void save_SINR(SINRResp resp) {
		if (resp == null)
			return;
		String destination = System.getProperty("user.dir");
		int last_indes = destination.lastIndexOf(File.separator);
		...
		String fileName = temp_pre_fix + File.separator + "report" + File.separator + "batch_"
				+ Integer.toString(current_batch) + File.separator + "uav_SINR" + ".csv";
		...
		try {
			...
			uav_SINR_string.append(internal_time_step);
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.uavId());
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.sinr());
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.distance());
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.cqi());
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.enbId());
			uav_SINR_string.append('\n');
	...
	// save UAV Throughput in each time tick
	private void save_Throughput(ThroughputResp resp) {
		if (resp == null)
			return;
		String destination = System.getProperty("user.dir");
		int last_indes = destination.lastIndexOf(File.separator);
		...
		String fileName = temp_pre_fix + File.separator + "report" + File.separator + "batch_"
				+ Integer.toString(current_batch) + File.separator + "uav_Throughput" + ".csv";
		...
		try {
			...
			System.out.println("Uav id = " + resp.uavId());
			System.out.println("Throughput [Tx, Rx] = " + resp.throughput());
			System.out.println("No connection =  " + resp.noConnection());

			uav_SINR_string.append(internal_time_step);
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.uavId());
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.throughput());
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.timeStamp());
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.totalBytes());
			uav_SINR_string.append(',');
			uav_SINR_string.append(resp.noConnection());
			uav_SINR_string.append('\n');
