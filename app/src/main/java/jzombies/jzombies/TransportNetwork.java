...
/**
 * * Node for `TransportNetwork` is some specific location on map. Like, Syarac-
 * * -use University, Wegmans, etc.
 * * Connection(Directed) between Nodes means that an UAV must follow this conn-
 * * -ection to move from one node to another.
 * * Load from JSON file: configuration/transport_network/*
 */
public class TransportNetwork {
	...
	public TransportNetwork(String mode) {
		...
		if (mode == "load") {
			//* Load from transport_config.json to generate Nodes.
			airport_list = mytool.transport_network_config();
			//* Load from transport_connection.json to generate Connections.
