...
//		}
    ...
    public boolean FillOutArray(UAV curr_uav, int curr_batch) {
		...
		short conn_row_index = (short) (distance2Coordinate(this.geography, this.TopLeftY, this.TopLeftX, this.TopLeftY, conn_y) / (curr_uav.return_speed() * 5));
		short conn_column_index = (short) (distance2Coordinate(this.geography, this.TopLeftY, this.TopLeftX, conn_x, this.TopLeftX) / (curr_uav.return_speed() * 5));
		...
		int end_time = Math.abs(conn_row_index - end_row_index) + Math.abs(conn_column_index - end_column_index) + conn_time;
		...
		ArrayList<Node> turn_point_first = ASTAR_Routing(start_row_index, start_column_index, start_time, conn_row_index, conn_column_index, conn_time, curr_batch);
		...
		ArrayList<Node> turn_point_two = ASTAR_Routing(conn_row_index, conn_column_index, last_time, end_row_index, end_column_index, conn_time, curr_batch);		
...
class HCostCalculator {
	...
	public double calcHCost(float[] sinrX, float probX, double distanceToAttachedBS,
		...
		double res = prob * sinr + (1.0 - prob) * staticSINR;
