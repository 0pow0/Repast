...
public class UAVmanagement {
	...
	public UAVmanagement(Geography<Object> geography, GeometryFactory fac, ArrayList<UAV> uavs_list, Map<String, ArrayList<Double>> airport_list, 
			Map<String, terrain_node> terrain_start_location, Map<String, terrain_node> terrain_end_location, int current_batch, int uav_generation_rate, boolean if_straight, boolean if_routing, int simulation_time, int c_seed, long num_ch) {
...
//		my_Deconfliction = new Deconfliction(geography, 1, -111.266107, 48.994677, -109.805044, 48.027456, simulation_time); 
//		my_Deconfliction = new Deconfliction(geography, 1, -111.266107, 48.994677, -111.035044, 47.907456, simulation_time); 
...
//		    //////////////////////////////////////////////////////////////////
		...
		if(this.internal_time_step % uav_generation_rate == 0) {
			...
			for(String area_name : this.terrain_start_location.keySet()) {
				...
				ArrayList<Double> start_coordinate_pair = area_generate_start_points(terrain_start_location.get(area_name),this.seed);
...
//						unmanned = false;
//					}
					UAV temp_uav = new UAV(geography, 400, speed, this.if_straight, this.if_routing, this.internal_time_step, 0, 0, true, "probability", null, current_batch, level, unmanned, -1, this.num_ch);
...
//		                short start_x = (short) (distance2Coordinate(geography, 48.994677,-111.266107, start_coordinate_pair.get(1), -111.266107)  / (18 * 5));
//		                short start_y = (short) (distance2Coordinate(geography, 48.994677,-111.266107, 48.994677, start_coordinate_pair.get(0))  / (18 * 5));
//		                short end_x = (short) (distance2Coordinate(geography, 48.994677,-111.266107, end_coordinate_pair.get(1), -111.266107)  / (18 * 5));
//		                short end_y = (short) (distance2Coordinate(geography, 48.994677,-111.266107, 48.994677, end_coordinate_pair.get(0))  / (18 * 5));
//		                
//		                T_distance = Math.sqrt(Math.pow((Math.abs(start_x - end_x) - 2) * 90, 2) + Math.pow((Math.abs(start_y - end_y) - 2) * 90, 2));
////		                System.out.print("*******************************\n");
...
//	                	System.out.println("with routing");
		                ...
		                temp_uav.set_connection_coordinate_pair(connection_coordinate_pair);
//		                temp_uav.stop_x = (short) (distance2Coordinate(geography, -76.2597052, 43.0802922, -76.2597052, connection_coordinate.get(1))  / (18 * 5));;
//		                temp_uav.stop_y = (short) (distance2Coordinate(geography, -76.2597052, 43.0802922, connection_coordinate.get(0), 43.0802922)  / (18 * 5));
	                	if_routing_success = my_Deconfliction.FillOutArray(temp_uav, current_batch);
	                } else {
//	                	System.out.println("without routing");
	                	if_routing_success = true;
